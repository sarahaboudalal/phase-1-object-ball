function gameObject(){
    return {
     home:{
        teamName: "Brooklyn Nets", 
        colors: ["Black", "White"], 
        players: {
            "Alan Anderson": { 
            number: 0,
            shoe: 16,
            points: 22,
            rebounds: 12,
            assists: 12,
            steals: 3,
            blocks: 1,
            slamDunks: 1,
        },
            "Reggie Evans": {
            number: 30,
            shoe: 14,
            points: 12,
            rebounds: 12,
            assists: 12,
            steals: 12,
            blocks: 12,
            slamDunks: 7,
        },
            "Brook Lopez": {
            number: 11,
            shoe: 17,
            points: 17,
            rebounds: 19,
            assists: 10,
            steals: 3,
            blocks: 1,
            slamDunks: 15,
        },
            "Mason Plumlee": {
            number: 1,
            shoe: 19,
            points: 26,
            rebounds: 12,
            assists: 6,
            steals: 3,
            blocks: 8,
            slamDunks: 5
        },
            "Jason Terry": {
            number: 31,
            shoe: 15,
            points: 19,
            rebounds: 2,
            assists: 2,
            steals: 4,
            blocks: 11,
            slamDunks: 1
        },
     },},
     away:{
        teamName: "Charlotte Hornets", 
        colors: ["Turquoise", "Purple"], 
        players: {
            "Jeff Adrien": {
            number: 4,
            shoe: 18,
            points: 10,
            rebounds: 1,
            assists: 1,
            steals: 2,
            blocks: 7,
            slamDunks: 2,
        },
            "Bismak Biyombo": {
            number: 0,
            shoe: 16,
            points: 12,
            rebounds: 4,
            assists: 7,
            steals: 7,
            blocks: 15,
            slamDunks: 10,
        },
            "DeSagna Diop": {
            number: 2,
            shoe: 14,
            points: 24,
            rebounds: 12,
            assists: 12,
            steals: 4,
            blocks: 5,
            slamDunks: 5,
        },
            "Ben Gordon": {
            number: 8,
            shoe: 15,
            points: 33,
            rebounds: 3,
            assists: 2,
            steals: 1,
            blocks: 1,
            slamDunks: 0,
        },
            "Brendan Haywood": {
            number: 33,
            shoe: 15,
            points: 6,
            rebounds: 12,
            assists: 12,
            steals: 22,
            blocks: 5,
            slamDunks: 12,
        },
     },
     },
    }
}

// function homeTeamName() {
//     let obj = gameObject()
//     return obj['home']['teamName']
//   }
  
//   console.log(homeTeamName())
function homeTeam (){
  return gameObject().home;
}

function awayTeam (){
   return gameObject().away;
}

function players(){
  // const game = gameObject()
  // return {...homePlayers, ...awayPlayers}
  return Object.assign({}, homeTeam().players, awayTeam().players);
}

  function numPointsScored(playerInput){
  //  for (const playerName in players()){
  //   if (playerName === playerInput){
  //     return players()[playerName].points
  //    }
  //   }
  // let points;
  // const playerArrays = Object.entries(players());
  // const player = playerArrays.find(playerArray => playerArray[0] === playerInput)
  return players()[playerInput].points
   }

  console.log(numPointsScored("DeSagna Diop"))

  function shoeSize(playerInput){
    return players()[playerInput].shoe
  }
  console.log(shoeSize('Brendan Haywood'))

  function colors(){
    return Object.assign(homeTeam().colors, awayTeam().colors);
  }

  function teamColors(teamInput){
    return colors()
  }
  console.log(teamColors('Charlotte Hornets'))

  function teamNames(){
    const name = [homeTeam().teamName, awayTeam().teamName]
    return name;
  }
  // function teamNames(){
  //   return team()
  // }

  console.log(teamNames())

  const playerNumbers = (teamName) => {
    const teamHome = gameObject()['home']['teamName']
    const teamAway = gameObject()['away']['teamName']
    let team = {}
    let numbersArray = []
    if (teamHome === teamName) {
      team = gameObject()['home']['players']
    } else if (teamAway === teamName) {
      team = gameObject()['away']['players']
    } else {
      return 'Team not exists'
    }
    Object.entries(team).map((el) => numbersArray.push(el[1].number))
    return numbersArray
  }

  function playerStats(playerInput){
    return players()[playerInput]
  }

  console.log(playerStats('Brendan Haywood'))

  const bigShoeRebounds = () => {
    const home = gameObject()['home']['players']
    const away = gameObject()['away']['players']
    let max = 0,
      maxHome = 0,
      maxAway = 0
    Object.entries(home).map((el) => {
      if (el[1].rebound > maxHome) maxHome = el[1].rebound
    })
    Object.entries(away).map((el) => {
      if (el[1].rebound > maxAway) maxAway = el[1].rebound
    })
    if (maxHome > maxAway) max = maxHome
    else max = maxAway
    return max
  }